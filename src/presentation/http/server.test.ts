...
import { buildDependencies } from '../../main/container.js'
import { buildServer } from './server.js'

// Integração com o grafo REAL montado pelo composition root — exercita rotas,
// serialização dos schemas e error handler via `app.inject()` (sem abrir porta).
describe('HTTP', () => {
  let app: FastifyInstance

  beforeAll(async () => {
    app = await buildServer(buildDependencies(), { logger: false })
    await app.ready()
  })
  ...
  describe('GET /products/:id', () => {
    ...
    it('responde 404 no formato de erro padrão quando o id não existe', async () => {
      ...
      expect(res.json()).toEqual({
        error: { statusCode: 404, message: 'Produto 999 não encontrado' },
      })
    })
  })
  ...
  describe('documentação', () => {
    ...
    it('registra os schemas compartilhados como components reutilizáveis', async () => {
      const res = await app.inject({ method: 'GET', url: '/docs/json' })

      expect(Object.keys(res.json().components.schemas)).toEqual(
        expect.arrayContaining(['Product', 'ProductResponse', 'ErrorResponse'])
      )
    })
  })
})
